...
async function createGoogleCalendarSchema(): Promise<void> {
  ...
  await database.begin(async (transaction) => {
    ...
    await transaction`
      ALTER TABLE google_calendar_connection
        ADD COLUMN IF NOT EXISTS busy_sync_from timestamptz,
        ADD COLUMN IF NOT EXISTS busy_sync_to timestamptz
    `;
    await transaction`
...
export async function ensureGoogleCalendarSchema(): Promise<void> {
  if (!schemaPromise) {
    schemaPromise = createGoogleCalendarSchema().catch((error) => {
